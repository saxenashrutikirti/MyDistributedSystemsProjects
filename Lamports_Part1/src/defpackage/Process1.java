...
public class Process1 extends Thread {
	...
	void threadForReceiving() throws IOException {
		...
		for (i = 0; i < 27; i++) {
			...
			clockProcess1++;// The logical is also increased when receiving the data because it is another
							// operation
			...
			dataArray[i] = data;
			time[i] = temp;
			if (time[i] > clockProcess1)// Checking if the time it occured is greater than the current logical clock
			// If so, then the logical clock is updated
			{
				clockProcess1 = time[i] + 1;
			}
			logicalTimeArray[i] = clockProcess1;
		}
	...
	void threadForSending() throws IOException, InterruptedException {
		...
		for (i = 0; i < length; i++) {
			...
			String stringForSending = "Process 1: ";
			s = stringForSending.concat(strArr[i]);
			s = s.concat("," + clockProcess1);
	...
	public static void main(String args[]) throws IOException {
		...
		br.readLine();
		Process1 T1 = new Process1("Sender");
		T1.start();

		Process1 T2 = new Process1("Receiver");
		T2.start();
	}
