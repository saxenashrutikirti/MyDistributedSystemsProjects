...
public class Process3 extends Thread {
	...
	void threadForReceiving() throws IOException {
		...
		for (i = 0; i < 27; i++) {
			...
			clockProcess3++;// The logical is also increased when receiving the data because it is another
							// operation
			...
			dataArray[i] = data;
			time[i] = temp;
			if (time[i] > clockProcess3)// Checking if the time it occured is greater than the current logical clock
			// If so, then the logical clock is updated
			{
				clockProcess3 = time[i] + 1;
			}
			logicalTimeArray[i] = clockProcess3;
		}
	...
	void threadForSending() throws IOException, InterruptedException {
		...
		for (i = 0; i < length; i++) {
			...
			String stringForSending = "Process 3: ";
			s = stringForSending.concat(strArr[i]);
			s = s.concat("," + clockProcess3);
	...
	public static void main(String args[]) throws IOException {
		...
		br.readLine();
		Process3 T1 = new Process3("Sender");
		T1.start();

		Process3 T2 = new Process3("Receiver");
		T2.start();
	}
